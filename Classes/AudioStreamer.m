...
//  ---------------------------------------------------------------------------
//  
//  Code for starting the stream from an offset from http://www.saygoodnight.com/?p=14
//  ---------------------------------------------------------------------------
//  
//  Minor modifications by Ben Baron to suit the iSub project.
//
...
{
	//
	// Overriding the interruption handling code as it is causing crashes
	//AudioStreamer* streamer = (AudioStreamer *)inClientData;
	//[streamer handleInterruptionChangeToState:inInterruptionState];
	//
	iSubAppDelegate *appDelegate = (iSubAppDelegate *)[[UIApplication sharedApplication] delegate];
	[appDelegate.streamer handleInterruptionChangeToState:inInterruptionState];
}
#endif
...
- (BOOL)openFileStream
{
	@synchronized(self)
	{
		...
		//NSLog(@"CFStringRef byteOffset = %@", byteOffset);
...
- (void)startWithOffsetInSecs:(UInt32) offsetInSecs
{
	...
	if (self.bitRate < 1000)
		self.offsetStart = ( self.bitRate * 125 * offsetInSecs ) + thisFileDataOffset;
	else
...
// THIS ONE WORKS IN SIMULATOR BUT FLAKES OUT ON DEVICE, RETURNS BAD VALUES EVEN THOUGH VARIABLES INSIDE METHOD ARE CORRECT
/*
- (double)progress
{
	@synchronized(self)
	{
		if (sampleRate > 0 && ![self isFinishing])
		{
			...
			NSLog(@"streamer.progress: %f", progress);
			return progress;
		}
	}
	return lastProgress;
...
- (double)progress
{
	...
	@synchronized(self)
	{
		if (sampleRate > 0 && ![self isFinishing])
		{
			if (state != AS_PLAYING && state != AS_PAUSED && state != AS_BUFFERING)
			{
				appDelegate.streamerProgress = lastProgress;
				return lastProgress;
			}
			...
			// (i.e. whenever the song info panel is in view) and caused the player to skip to the next song. 
			if (err)
			{
				return lastProgress;
				//[self failWithErrorCode:AS_GET_AUDIO_TIME_FAILED];
			...
			lastProgress = progress;
			appDelegate.streamerProgress = progress;
			return progress;
		}
	}
	appDelegate.streamerProgress = lastProgress;
...
- (void)handleInterruptionChangeToState:(AudioQueuePropertyID)inInterruptionState
{
	...
	if (inInterruptionState == kAudioSessionBeginInterruption)
	{
		...
		{	
			[self progress];
			appDelegate.seekTime += appDelegate.streamerProgress;
		}
	...
	else if (inInterruptionState == kAudioSessionEndInterruption)
	{
		...
		if(appDelegate.isPlaying)
		{
			[self startWithOffsetInSecs:(UInt32) appDelegate.seekTime];
		}
